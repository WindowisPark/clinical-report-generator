-- Generated from: screen_clinical_trial_candidates.yaml
-- Parameters used: {'exclusion_window_months': 10, 'min_age': 35, 'max_age': 35, 'snapshot_dt': '2025-09-01'}

-- 임상시험 후보자 선정을 위한 복잡한 스크리닝 (연령, 지역, 상세 제외 기준 적용)
-- 파라미터: exclusion_window_months = 10, min_age = 35, max_age = 35
WITH params AS (
  SELECT
    10 AS ex_window_months,                                -- 파라미터 1: 배제 조건 기간(개월)
    ADD_MONTHS(CURRENT_DATE, -10) AS dt_ex_window
),
  ...
  SELECT
    ...
    CASE
      WHEN p.birthday RLIKE '^[0-9]{8}$'
       ...
       AND NOT (SUBSTRING(p.birthday,5,2) = '02' AND CAST(SUBSTRING(p.birthday,7,2) AS INT) > 29)
       AND NOT (SUBSTRING(p.birthday,5,2) = '02' AND SUBSTRING(p.birthday,7,2) = '29' AND NOT ((CAST(SUBSTRING(p.birthday,1,4) AS INT) % 400 = 0) OR (CAST(SUBSTRING(p.birthday,1,4) AS INT) % 4 = 0 AND CAST(SUBSTRING(p.birthday,1,4) AS INT) % 100 <> 0)))
...
base_users_filtered AS (
  ...
  FROM base_users
  WHERE age_years BETWEEN 35 AND 35 -- 파라미터 2, 3
),
