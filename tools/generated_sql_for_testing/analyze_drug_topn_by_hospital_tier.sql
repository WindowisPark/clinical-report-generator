-- Generated from: analyze_drug_topn_by_hospital_tier.yaml
-- Parameters used: {'disease_keyword': '당뇨병', 'drug1_ingredient': '아스피린', 'drug2_ingredient': '이부프로펜', 'snapshot_dt': '2025-09-01'}

-- 특정 질환을 가진 환자들 중 특정 약물 조합을 처방받은 환자들의 병원급별 분석
-- 파라미터: disease_keyword = 당뇨병, drug1_ingredient = 아스피린, drug2_ingredient = 이부프로펜

WITH base_diabetes AS (
  SELECT DISTINCT
    pd.user_id,
  ...
  WHERE bt.res_disease_name LIKE CONCAT('%', '당뇨병', '%')
...
drug_flags AS (
  SELECT
    ...
    MAX(CASE WHEN LOWER(res_ingredients) LIKE CONCAT('%', LOWER('아스피린'), '%') THEN 1 ELSE 0 END) AS has_drug1,
    MAX(CASE WHEN LOWER(res_ingredients) LIKE CONCAT('%', LOWER('이부프로펜'), '%') THEN 1 ELSE 0 END) AS has_drug2
