-- Generated from: analyze_optimized_rwe_patterns.yaml
-- Parameters used: {'disease_name_keyword': '당뇨병', 'start_date': '2022-01-01', 'end_date': '2024-12-31', 'min_pattern_size': 10, 'snapshot_dt': '2025-09-01'}

-- 개선된 Real-World Evidence (RWE) 분석
-- 파라미터: disease_code_prefix = , start_date = 2022-01-01, end_date = 2024-12-31, min_pattern_size = 10
...
WITH filtered_treatments AS (
    ...
    WHERE bt.res_disease_name LIKE '%당뇨병%' -- 파라미터 1
        ...
        AND bt.res_total_amount REGEXP '^[0-9]+$'  -- 정규식 최적화
...
patient_demographics AS (
    SELECT
        ...
        YEAR(pv.first_visit) -
        CASE
            WHEN LENGTH(pv.birthday) >= 8 THEN CAST(SUBSTRING(pv.birthday, 1, 4) AS INT)
            WHEN LENGTH(pv.birthday) >= 6 THEN
                CASE
                    WHEN CAST(SUBSTRING(pv.birthday, 1, 2) AS INT) <= 30 THEN 2000 + CAST(SUBSTRING(pv.birthday, 1, 2) AS INT)
                    ELSE 1900 + CAST(SUBSTRING(pv.birthday, 1, 2) AS INT)
                END
            ELSE NULL
...
HAVING COUNT(*) >= 10  -- 최소 패턴 크기 이상만 표시
