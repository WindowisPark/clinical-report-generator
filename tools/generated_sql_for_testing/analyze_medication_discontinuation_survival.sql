-- Generated from: analyze_medication_discontinuation_survival.yaml
-- Parameters used: {'disease_name_keyword': '당뇨병', 'start_date': '2022-01-01', 'end_date': '2024-12-31', 'target_therapeutic_class': 'test_string', 'analysis_period_days': 730, 'snapshot_dt': '2025-09-01'}

-- 약물 중단율 생존분석 (치료클래스 + 위험요인)
-- 파라미터: disease_name_keyword = 당뇨병, start_date = 2022-01-01, end_date = 2024-12-31, target_therapeutic_class = test_string, analysis_period_days = 730
...
patient_covariates AS (
    SELECT
        ...
        SUM(CAST(REGEXP_REPLACE(bt.res_total_amount, '[^0-9]', '') AS INT)) as total_medical_cost,
        -- 위험요인 분류
        CASE
        ...
        CASE
            WHEN SUM(CAST(REGEXP_REPLACE(bt.res_total_amount, '[^0-9]', '') AS INT)) >= 100000000 THEN 'High_Cost'
            WHEN SUM(CAST(REGEXP_REPLACE(bt.res_total_amount, '[^0-9]', '') AS INT)) >= 50000000 THEN 'Medium_Cost'
            ELSE 'Low_Cost'
...
target_prescriptions AS (
    ...
    WHERE tc.therapeutic_class = 'test_string'
...
discontinuation_events AS (
    ...
    WHERE DATEDIFF(tp.prescription_date, tp.first_prescription_date) <= 730
)
