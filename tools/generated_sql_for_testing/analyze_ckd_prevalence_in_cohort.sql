-- Generated from: analyze_ckd_prevalence_in_cohort.yaml
-- Parameters used: {'cohort_disease_name': 'test_string', 'cohort_disease_code': 'test_string', 'snapshot_dt': '2025-09-01'}

-- 특정 질환 코호트 내에서 만성 신장병(CKD) 유병률 분석 (최적화 버전)
-- GROUP BY와 조건부 집계를 사용하여 basic_treatment 테이블 스캔을 1회로 줄임
-- 파라미터: cohort_disease_name = test_string, cohort_disease_code = test_string
...
WITH user_flags AS (
  SELECT
    ...
    MAX(CASE
      WHEN UPPER(COALESCE(res_disease_name,'')) LIKE UPPER('%test_string%')
        OR UPPER(COALESCE(res_disease_name,'')) LIKE '%test_string%'
      THEN 1 ELSE 0 END
    ) as is_in_cohort,
