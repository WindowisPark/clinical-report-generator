-- Generated from: analyze_obesity_patients_with_bmi.yaml
-- Parameters used: {'obesity_keyword': '비만', 'snapshot_dt': '2025-09-01'}

-- 비만 진단 + 건강검진 보유자 매칭 & 최신 BMI 부착
-- 파라미터: obesity_keyword = 비만
...
WITH obese_users AS (
    ...
    SELECT
        ...
        MIN(
            CASE
                ...
                AND CAST(SUBSTRING(bt.res_treat_start_date,1,4) AS INT) BETWEEN 1900 AND YEAR(current_date())
                AND CAST(SUBSTRING(bt.res_treat_start_date,5,2) AS INT) BETWEEN 1 AND 12
                AND CAST(SUBSTRING(bt.res_treat_start_date,7,2) AS INT) BETWEEN 1 AND 31
                -- 윤년 2/29 검증
                AND NOT (
                    CAST(SUBSTRING(bt.res_treat_start_date,5,2) AS INT) = 2
                    AND CAST(SUBSTRING(bt.res_treat_start_date,7,2) AS INT) = 29
                    AND MOD(CAST(SUBSTRING(bt.res_treat_start_date,1,4) AS INT),4) <> 0
                )
                -- 2월 30/31 제거
                AND NOT (
                    CAST(SUBSTRING(bt.res_treat_start_date,5,2) AS INT) = 2
                ...
                -- 4,6,9,11월 31일 제거
                AND NOT (
                    CAST(SUBSTRING(bt.res_treat_start_date,5,2) AS INT) IN (4,6,9,11)
                    AND CAST(SUBSTRING(bt.res_treat_start_date,7,2) AS INT) = 31
                )
    ...
    WHERE (UPPER(bt.res_disease_code) LIKE 'E66%' OR bt.res_disease_name LIKE CONCAT('%', '비만', '%'))
...
hc_norm AS (
    ...
    SELECT
        ...
        CASE
            ...
            AND CAST(SUBSTRING(h.res_checkup_date,1,4) AS INT) BETWEEN 1900 AND YEAR(current_date())
            AND CAST(SUBSTRING(h.res_checkup_date,5,2) AS INT) BETWEEN 1 AND 12
            AND CAST(SUBSTRING(h.res_checkup_date,7,2) AS INT) BETWEEN 1 AND 31
            AND NOT (
                CAST(SUBSTRING(h.res_checkup_date,5,2) AS INT) = 2
                AND CAST(SUBSTRING(h.res_checkup_date,7,2) AS INT) = 29
                AND MOD(CAST(SUBSTRING(h.res_checkup_date,1,4) AS INT),4) <> 0
            )
            AND NOT (
                CAST(SUBSTRING(h.res_checkup_date,5,2) AS INT) = 2
                AND CAST(SUBSTRING(h.res_checkup_date,7,2) AS INT) > 29
            ...
            AND NOT (
                CAST(SUBSTRING(h.res_checkup_date,5,2) AS INT) IN (4,6,9,11)
                AND CAST(SUBSTRING(h.res_checkup_date,7,2) AS INT) = 31
            )
