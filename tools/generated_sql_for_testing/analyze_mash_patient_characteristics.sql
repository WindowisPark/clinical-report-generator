-- Generated from: analyze_mash_patient_characteristics.yaml
-- Parameters used: {'disease_keyword': '염증성 간질환', 't2d_keyword': '당뇨', 'dyslipidemia_keyword': '지질', 'hypertension_keyword': '고혈압', 'snapshot_dt': '2025-09-01'}

-- MASH 환자 특성 분석 (연령, 합병증, 병원 분포)
-- 파라미터: disease_keyword = 염증성 간질환
...
WITH mash_patients AS (
    ...
    WHERE bt.res_disease_name LIKE CONCAT('%', '염증성 간질환', '%')
...
with_comorb AS (
    SELECT
        ...
        a.last_treat_date,
        -- 합병증 동반 여부 플래그
        MAX(CASE WHEN bt.res_disease_code LIKE 'E11%' OR bt.res_disease_name LIKE '%당뇨%' THEN 1 ELSE 0 END) AS has_t2d,
        MAX(CASE WHEN bt.res_disease_code LIKE 'E78%' OR bt.res_disease_name LIKE '%지질%' OR bt.res_disease_name LIKE '%고지혈%' THEN 1 ELSE 0 END) AS has_dyslipidemia,
        MAX(CASE WHEN bt.res_disease_code LIKE 'I10%' OR bt.res_disease_name LIKE '%고혈압%' THEN 1 ELSE 0 END) AS has_hypertension
    FROM with_age a
...
with_hospital AS (
    ...
    LEFT JOIN basic_treatment bt
        ...
        AND bt.res_disease_name LIKE CONCAT('%', '염증성 간질환', '%')
        AND bt.deleted = FALSE
