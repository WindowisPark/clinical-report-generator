...
WITH recent_neuropathy_patients AS (
  ...
  WHERE bt.res_disease_name LIKE CONCAT('%', '{{ neuropathy_keyword }}', '%')             -- 파라미터 1
    AND bt.res_disease_name NOT LIKE CONCAT('%', '{{ neuropathy_exclude_keyword }}', '%') -- 파라미터 2
...
diabetes_patients AS (
  SELECT DISTINCT bt.user_id
  FROM `basic_treatment` bt
  WHERE bt.res_disease_name LIKE CONCAT('%', '{{ exclusion_disease_keyword }}', '%')  -- 파라미터 3
    AND bt.deleted = FALSE
),
