...
-- 파라미터: disease_keyword = {{ disease_keyword }}, drug1_ingredient = {{ drug1_ingredient }}, drug2_ingredient = {{ drug2_ingredient }}
...
drug_flags AS (
  SELECT
    ...
    MAX(CASE WHEN LOWER(res_ingredients) LIKE CONCAT('%', LOWER('{{ drug1_ingredient }}'), '%') THEN 1 ELSE 0 END) AS has_drug1,
    MAX(CASE WHEN LOWER(res_ingredients) LIKE CONCAT('%', LOWER('{{ drug2_ingredient }}'), '%') THEN 1 ELSE 0 END) AS has_drug2
...
hospital_grade_raw AS (
  ...
  FROM merged m
  JOIN basic_treatment bt
    ON m.user_id = bt.user_id AND m.res_hospital_name = bt.res_hospital_name
