-- MPR 복약순응도 지표 계산 분석
-- 파라미터: disease_name_keyword = {{ disease_name_keyword }}, start_date = {{ start_date }}, end_date = {{ end_date }}, target_ingredient = {{ target_ingredient }}, analysis_period_days = {{ analysis_period_days }}

-- 1단계: 대상 환자 및 최초 진단일 수집
WITH target_patients AS (
    SELECT DISTINCT
        bt.user_id,
        MIN(TO_DATE(bt.created_at)) as first_diagnosis_date
...
medication_prescriptions AS (
    ...
    WHERE (
        ...
        AND CAST(pd.res_total_dosing_days AS INTEGER) <= 180  -- 합리적 범위 제한
...
patient_analysis_periods AS (
    SELECT
        ...
        DATE_ADD(tp.first_diagnosis_date, CAST({{ analysis_period_days }} AS INTEGER)) as analysis_end_date,
        CAST({{ analysis_period_days }} AS INTEGER) as total_analysis_days
...
SELECT
    CASE
        WHEN '{{ target_ingredient }}' = '' THEN 'MPR 복약순응도 분석 (전체 성분)'
        ELSE CONCAT('MPR 복약순응도 분석 (', '{{ target_ingredient }}', ')')
