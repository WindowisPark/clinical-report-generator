...
hospital_metrics AS (
    ...
    HAVING COUNT(DISTINCT bt.user_id) >= GREATEST({{ min_patient_count|int / 10 }}, 3)  -- 샘플 기준 최소값
)
...
SELECT
    ...
    ROW_NUMBER() OVER (ORDER BY
        ROUND(
            (patient_retention_rate * {{ effectiveness_weight }}) +
            ((100 - major_change_rate) * {{ satisfaction_weight }}) +
            (optimal_interval_rate * {{ accessibility_weight }}) +
            (CASE WHEN active_days > 100 THEN 80 ELSE 60 END * 0.1)
        , 1) DESC) as recommendation_rank,
    res_hospital_code,
    name as medical_facility_name,
    medical_facility_type_code,
    full_address as location_address,
    ...
    -- 등급 (파라미터 기반)
    CASE
        WHEN ROUND((patient_retention_rate * {{ effectiveness_weight }}) +
                   ((100 - major_change_rate) * {{ satisfaction_weight }}) +
                   (optimal_interval_rate * {{ accessibility_weight }}) +
