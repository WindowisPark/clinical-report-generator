WITH base_visits AS (
  ...
  WHERE bt.res_disease_name LIKE '%' || '{{ disease_keyword }}' || '%' -- 파라미터 1
),
...
drug_flags AS (
  SELECT
    ...
    MAX(CASE WHEN LOWER(res_ingredients) LIKE '%' || lower('{{ drug1_keyword }}') || '%' THEN 1 ELSE 0 END) AS has_drug1, -- 파라미터 2
    MAX(CASE WHEN LOWER(res_ingredients) LIKE '%' || lower('{{ drug2_keyword }}') || '%' THEN 1 ELSE 0 END) AS has_drug2  -- 파라미터 3
